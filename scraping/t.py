...
def check_percent(key,num,per):
    ...
    with open('nikkei.csv','r',newline='',encoding='utf-8') as f:
        ...
        for l in r:
            ...
            if(tmp == 6):
                if(l[5].replace(',','').replace('‥','')  == "--"):
                    continue
                tmp_brand = int(l[1])
                tmp_data = float(l[5].replace(',','').replace('‥','') )
                tmp_flag = True
                ...
                if(l[5].replace(',','').replace('‥','')  == "--"):
                    tmp_flag = False
                    continue
                if(tmp_flag and tmp_brand == int(l[1]) and tmp_data - tmp_data/(num * per) < float(l[5].replace(',','').replace('‥','') ) ):
                    tmp_data = float(l[5].replace(',','.').replace('‥','') )
                else:
                    tmp_flag = False
                ...
                if(l[5].replace(',','').replace('‥','')  == "--"):
                    tmp_flag = False
                    continue
                if(tmp_flag and tmp_brand == int(l[1]) and tmp_data - tmp_data/ (num * (1 - per)) < float(l[5].replace(',','').replace('‥','') ) ):
                    if(getStockdata(int(l[1]),float(l[5].replace(',','').replace('‥','') ),key) == 1):
                        correct += 1
                    elif(getStockdata(int(l[1]),float(l[5].replace(',','').replace('‥','') ),key) == 2):
                        false += 1
                tmp_flag = False
            #print(l[1].rstrip('\n')+"番の企業の"+l[3]+"の時期のPERは"+l[8])
