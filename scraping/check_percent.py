        ...
        for l in r:
            ...
            if(tmp == 6):
                ...
                if(l[7].replace(',','.') == "--"):
                    tmp_flag = False
                    continue
                if(tmp_flag and tmp_brand == int(l[1]) and tmp_data - tmp_data/(num * per) < float(l[7].replace(',','.')) ):
                    tmp_data = float(l[7].replace(',','.'))
                else:
                    tmp_flag = False
                ...
                if(l[7].replace(',','.') == "--"):
                    tmp_flag = False
                    continue
                if(tmp_flag and tmp_brand == int(l[1]) and tmp_data - tmp_data/ (num * (1 - per)) < float(l[7].replace(',','.')) ):
...
for i in np.arange(1.55,1.70,0.01):
    for j in np.arange(0.7,0.9,0.01):
